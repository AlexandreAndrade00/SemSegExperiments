...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, dataset_path: str, scale: float = 1):
        ...
        self.transformer = transforms.Compose(
            [transforms.ToTensor(),
             transforms.Resize((int(self._IMG_H * scale), int(self._IMG_W * scale)),
                               transforms.InterpolationMode.BICUBIC)]) if scale != 1 else transforms.ToTensor()

        self.target_transformer = transforms.Compose(
            [transforms.PILToTensor(),
             transforms.Resize((int(self._IMG_H * scale), int(self._IMG_W * scale)),
                               transforms.InterpolationMode.NEAREST_EXACT),
             transforms.Lambda(lambda x: x[0, :, :])]) if scale != 1 else transforms.Compose(
            ...
            with scandir(images_set_dir) as it1:
                for city in it1:
                    with scandir(city.path) as it2:
                        city_ids = [join(dataset_set, city.name, '_'.join(image.name.split('_')[0:3])) for image in it2]
    ...
    def __getitem__(self, idx):
        ...
        assert len(image_file) == 1, f'Either no image or multiple images found for the ID {image_id}: {image_file}'
        assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {image_id}: {mask_file}'
